...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Reveal */
  .reveal { opacity:0; transform:translateY(44px); transition:opacity 0.95s cubic-bezier(.2,.8,.3,1), transform 0.95s cubic-bezier(.2,.8,.3,1); }
  .reveal.in { opacity:1; transform:translateY(0); }
  .reveal-l { opacity:0; transform:translateX(-44px); transition:opacity 0.95s cubic-bezier(.2,.8,.3,1), transform 0.95s cubic-bezier(.2,.8,.3,1); }
  .reveal-l.in { opacity:1; transform:translateX(0); }
  .reveal-r { opacity:0; transform:translateX(44px); transition:opacity 0.95s cubic-bezier(.2,.8,.3,1), transform 0.95s cubic-bezier(.2,.8,.3,1); }
  ...
  .vrow:hover { background:rgba(255,255,255,0.018); padding-left:14px!important; transition:padding 0.35s ease,background 0.35s ease; }
  ...
  /* Sec tag */
  .sec-tag::before { content:''; display:inline-block; width:36px; height:1px; background:var(--gold); vertical-align:middle; margin-right:14px; }
...
function useHeroScene(canvasRef:React.RefObject<HTMLCanvasElement|null>) {
  useEffect(()=>{
    ...
    const kl=new THREE.DirectionalLight(0xc9a84c,1.4); kl.position.set(6,6,4); scene.add(kl);
    const fl=new THREE.DirectionalLight(0xffffff,0.3); fl.position.set(-4,-2,3); scene.add(fl);
    ...
    const bMat=new THREE.MeshStandardMaterial({color:0x0c0c0c,metalness:0.98,roughness:0.05});
    const body=new THREE.Mesh(new THREE.BoxGeometry(3.4,0.72,1.5,6,2,3),bMat); carGroup.add(body);
...
const Hero = ({settings,onViewCollection}:{settings:Record<string,string>;onViewCollection:()=>void}) => {
  ...
  return (
    <section id="hero" style={{position:"relative",height:"100vh",minHeight:700,display:"flex",alignItems:"center",overflow:"hidden",background:"radial-gradient(ellipse at 70% 50%, #0f0c08 0%, #060606 60%)"}}>
      ...
      <video autoPlay muted loop playsInline style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0}}>
...
const Fleet = ({vehicles,onSelect}:{vehicles:Vehicle[];onSelect:(v:Vehicle)=>void}) => {
  ...
  return (
    <section id="fleet" style={{padding:"120px 0",background:"var(--black)"}}>
      <div style={{maxWidth:1400,margin:"0 auto",padding:"0 60px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          ...
          {shown.map((v,i)=>(
            <div key={v.id} className="vrow reveal" style={{display:"grid",gridTemplateColumns:"52px 1fr 120px 100px 100px 180px 110px",alignItems:"center",gap:"0 36px",padding:"26px 0",borderBottom:"1px solid var(--border)",cursor:"pointer",transitionDelay:`${i*0.04}s`}} onClick={()=>onSelect(v)}>
